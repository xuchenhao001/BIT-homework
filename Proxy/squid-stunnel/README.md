...
```bash
$ sed -i "s/http_port 3128/https_port 443 cert=\/root\/.squid\/cert.crt key=\/root\/.squid\/cert.key/" /usr/local/squid/etc/squid.conf
$ sed -i "s/http_access deny all/http_access allow all/" /usr/local/squid/etc/squid.conf
```
